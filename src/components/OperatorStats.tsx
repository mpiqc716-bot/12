...
export function OperatorStats({ records, currentUser, appUsers }: OperatorStatsProps) {
  // Extract all unique usernames across users database + pipe logs
  const allOperatorNames = useMemo(() => {
    ...
    records.forEach(r => {
      if (r.operatorUsername) names.add(r.operatorUsername);
      if (r.dispatchedBy) names.add(r.dispatchedBy);
      if (r.steps) {
        Object.values(r.steps).forEach((s: any) => {
          if (s?.savedBy) names.add(s.savedBy);
        });
      }
    });
  ...
  // Date filters for PDF / analysis
  const [monthFilter, setMonthFilter] = useState<string>("All");

  // Process master pipe statistics per pipe for the chosen operator
  const pipeStatistics = useMemo(() => {
    const isAllOps = selectedOperator === "ALL";

    // Filter relevant pipes first: created or controlled by the operator
    const relevantPipes = records.filter(pipe => {
    ...
    return relevantPipes.map(pipe => {
      ...
      Object.entries(pipe.steps || {}).forEach(([stepNoStr, step]: [string, any]) => {
        ...
        // Was this step controlled by the selected operator (or ALL)?
        if (isAllOps || step.savedBy === selectedOperator) {
          stepsControlledSet.add(stepNo);
        }

        // Check if this step failed quality checks or NCR
      ...
      // Also check if pipe creation was at step 1
      if ((isAllOps || pipe.operatorUsername === selectedOperator) && pipe.steps?.[1]) {
        stepsControlledSet.add(1);
      }

      // Determine STATUS: NON CONFORM vs LIBERATED vs ON HOLD
  ...
  const exportToCSV = () => {
    const rows = [
      ["Operator Pipe Statistics Audit Export"],
      ["Target Operator", selectedOperator],
      ["Export Date", new Date().toLocaleString()],
      [""],
      ["PIPE N°", "Project Work Order", "Pipe Type", "Created By", "STEPS CONTROLLED", "STEPS FAILED", "STATUS"],
    ];

    filteredPipeStats.forEach(item => {
      rows.push([
        item.pipeId,
    ...
    link.setAttribute("download", `Operator_Pipe_Statistics_${selectedOperator}_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div className="space-y-6 animate-fade-in font-sans">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 p-6 rounded-3xl border border-indigo-900/40 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              <span className="bg-indigo-500/20 border border-indigo-400/30 text-indigo-300 font-extrabold uppercase tracking-widest text-[9.5px] px-2.5 py-0.5 rounded-full flex items-center gap-1">
                <UserCheck className="w-3 h-3 text-indigo-400" />
                Multi-Operator Quality Control
              </span>
              <span className="bg-emerald-500/20 border border-emerald-400/30 text-emerald-300 font-extrabold uppercase tracking-widest text-[9.5px] px-2.5 py-0.5 rounded-full">
                Real-Time Pipe Status Audit
              </span>
            </div>

            <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white flex items-center gap-2">
              <User className="w-7 h-7 text-indigo-400" />
              Operator Statistics & Pipe Control Ledger
            </h1>
            <p className="text-xs sm:text-sm text-indigo-200/80 mt-1 max-w-2xl">
              Complete breakdown per pipe showing controlled inspection steps, failed steps, and live status (NON CONFORM, ON HOLD, or LIBERATED).
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            <button
              onClick={() => {
                try {
                  generateOperatorShiftReport(records, currentUser?.username || "admin", appUsers, {
                    selectedOperator,
                    statusFilter,
                    month: monthFilter,
                    searchQuery
              ...
              className="bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold text-xs px-4 py-2.5 rounded-2xl flex items-center gap-2 shadow-md hover:shadow-lg transition active:scale-95 cursor-pointer uppercase tracking-wider"
            >
        ...
        {/* Operator Selector & Filter Controls Bar */}
        <div className="mt-6 pt-5 border-t border-indigo-900/50 grid grid-cols-1 md:grid-cols-4 gap-3 items-center">
          
          <div className="md:col-span-2">
            <label htmlFor="operator-select-dropdown" className="text-[10px] font-extrabold uppercase tracking-widest text-indigo-300 block mb-1">
            ...
            <div className="relative">
              <select
                ...
                className="w-full bg-slate-950 border border-indigo-500/30 text-sm font-black text-white rounded-xl px-3.5 py-2.5 outline-none focus:border-indigo-400 transition cursor-pointer shadow-inner appearance-none pr-8"
              >
                <option value="ALL">🌟 All Operators (Combined Factory Roster)</option>
                {allOperatorNames.map(op => (
                  <option key={op} value={op}>
                    👤 Operator: {op} {op === currentUser?.username ? " (You)" : ""}
                  </option>
                ))}
              </select>
          ...
          <div>
            <label htmlFor="date-month-filter" className="text-[10px] font-extrabold uppercase tracking-widest text-indigo-300 block mb-1">
              Month Horizon
            </label>
            <select
              ...
              className="w-full bg-slate-950 border border-indigo-500/30 text-xs font-bold text-slate-200 rounded-xl px-3 py-2.5 outline-none focus:border-indigo-400 transition cursor-pointer"
            >
          ...
          <div>
            <label htmlFor="search-input-operator-stats" className="text-[10px] font-extrabold uppercase tracking-widest text-indigo-300 block mb-1">
              Search Pipe N°
            </label>
            <div className="relative">
              <input
                id="search-input-operator-stats"
                type="text"
                placeholder="Search Pipe ID..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full bg-slate-950 border border-indigo-500/30 text-xs text-white rounded-xl pl-8 pr-3 py-2.5 outline-none focus:border-indigo-400 transition placeholder-slate-500"
              />
              <Search className="w-3.5 h-3.5 text-indigo-400 absolute left-2.5 top-3" />
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div 
          ...
          className={`p-5 rounded-2xl border transition cursor-pointer flex flex-col justify-between relative overflow-hidden ${
            statusFilter === "ALL" 
              ? "bg-slate-900 text-white border-slate-700 shadow-lg ring-2 ring-indigo-400" 
        ...
        >
          <div className="flex items-start justify-between">
            ...
            <Package className={`w-8 h-8 ${statusFilter === "ALL" ? "text-indigo-300" : "text-indigo-500"}`} />
          </div>
          <p className={`text-[10.5px] mt-3 pt-2 border-t ${statusFilter === "ALL" ? "border-slate-800 text-slate-300" : "border-slate-100 text-slate-500"}`}>
            Pipes associated with {selectedOperator === "ALL" ? "all operators" : selectedOperator}
          </p>
        </div>
        ...
        >
          ...
          <p className={`text-[10.5px] mt-3 pt-2 border-t ${statusFilter === "NON CONFORM" ? "border-rose-800 text-rose-200" : "border-slate-100 text-slate-500"}`}>
            Defects or NCR flags registered
          </p>
        </div>
        ...
        >
          ...
          <p className={`text-[10.5px] mt-3 pt-2 border-t ${statusFilter === "ON HOLD" ? "border-amber-800 text-amber-200" : "border-slate-100 text-slate-500"}`}>
            Inspection steps in progress / pending release
          </p>
        </div>
        ...
        <div className="p-5 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
          <div>
            <h3 className="font-black text-slate-900 text-base flex items-center gap-2">
              ...
              Pipe Control Audit Table
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Showing PIPE N°, STEPS CONTROLLED, STEPS FAILED, and STATUS for operator {selectedOperator === "ALL" ? "All Operators" : selectedOperator}.
            </p>
          </div>
          <span className="text-xs bg-indigo-100 text-indigo-800 font-bold px-3 py-1 rounded-full border border-indigo-200">
            {filteredPipeStats.length} Pipe Items
          </span>
        </div>

        {filteredPipeStats.length === 0 ? (
          <div className="p-12 text-center text-slate-400 space-y-2">
            <Package className="w-10 h-10 mx-auto text-slate-300" />
            <p className="font-bold text-slate-600 text-sm">No pipe records match the criteria for {selectedOperator}.</p>
          </div>
        ) : (
                  ...
                  return (
                    <React.Fragment key={item.pipeId}>
                      <tr 
                        ...
                        className={`hover:bg-indigo-50/50 transition cursor-pointer ${isExpanded ? "bg-indigo-50/70" : ""}`}
                      >
                        {/* 1. PIPE N° */}
                        <td className="p-4 pl-6">
                          <div className="flex items-center gap-2">
                            <strong className="text-base font-black text-indigo-950 font-mono tracking-tight">
                              {item.pipeId}
                            </strong>
                            ...
                            <div className="space-y-3">
                              <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                                <h4 className="text-xs font-extrabold text-indigo-300 uppercase tracking-wider flex items-center gap-1.5">
                                  ...
                                  Pipe N° {item.pipeId} — Complete Telemetry & Step Records
                                </h4>
                                    ...
                                    <div key={stepNo} className="bg-slate-950 p-3 rounded-xl border border-slate-800 text-xs space-y-1">
                                      <div className="flex items-center justify-between border-b border-slate-800 pb-1">
                                        ...
                                        <span className="text-[10px] text-slate-400 font-mono">
                                          By {step.savedBy || "System"} • {step.savedAt ? new Date(step.savedAt).toLocaleTimeString() : ""}
                                        </span>
                                      </div>
                                      ...
                                      {stepNo === 8 && (
                                        <p className="text-slate-300">Destination: <strong className="text-emerald-300">{fields.pipeDestination || "DP-COMMERCIAL"}</strong> | Inspector: {fields.inspectorName || selectedOperator}</p>
                                      )}
